...
const refuserDemande = async (req, res) => {
  try {
    ...
    if (!motif || !motif.trim()) {
      return error(res, 'Le motif de refus est obligatoire', 400);
    }

    const [rows] = await db.query(
      'SELECT statut, agent_id, numero_ordre, equipement_id FROM demandes_consignation WHERE id = ?', [id]
    );
    if (!rows.length) return error(res, 'Demande introuvable', 404);
    ...
    await db.query(
      `UPDATE demandes_consignation
       SET statut = 'rejetee', commentaire_rejet = ?, charge_id = ?, updated_at = NOW()
       WHERE id = ?`,
      [motif.trim(), charge_id, id]
    );

    const [chargeInfo] = await db.query(
      'SELECT prenom, nom FROM users WHERE id = ?', [charge_id]
    );
    const chargeNom = chargeInfo.length
      ? `${chargeInfo[0].prenom} ${chargeInfo[0].nom}`
    ...
    await envoyerPushNotification(
      [dem.agent_id],
      'Demande refusée',
      `${dem.numero_ordre} refusée — ${motif.trim()}`,
      { demande_id: id, statut: 'rejetee' }
    );
...
// ─────────────────────────────────────────────
// POST /charge/demandes/:id/mettre-en-attente
// Body: { motif, heure_reprise }
// ─────────────────────────────────────────────
const mettreEnAttente = async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE demandes_consignation
       SET statut = 'en_attente', charge_id = ?, updated_at = NOW(), commentaire_rejet = ?
       WHERE id = ?`,
...
const scannerCadenasLibre = async (req, res) => {
  try {
    ...
    if (!demande_id || !numero_cadenas || !mcc_ref) {
      return error(res, 'demande_id, numero_cadenas et mcc_ref sont requis', 400);
    }

    let [plans] = await db.query(
      'SELECT id FROM plans_consignation WHERE demande_id = ?', [demande_id]
    );
    ...
    if (plans.length === 0) {
      const [planResult] = await db.query(
        `INSERT INTO plans_consignation (demande_id, etabli_par, approuve_par, date_etabli, date_approuve, statut, remarques)
         VALUES (?, ?, ?, NOW(), NOW(), 'en_execution', 'Plan créé automatiquement par le chargé')`,
      ...
      plan_id = plans[0].id;
      await db.query(
        `UPDATE plans_consignation SET statut = 'en_execution', updated_at = NOW() WHERE id = ?`,
        [plan_id]
      );
    }

    const [lineCount] = await db.query(
      'SELECT MAX(numero_ligne) AS max_ligne FROM points_consignation WHERE plan_id = ?',
      [plan_id]
    );
    ...
    const [pointResult] = await db.query(
      `INSERT INTO points_consignation
       (plan_id, numero_ligne, repere_point, localisation, dispositif_condamnation, etat_requis, electricien_id, statut)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'consigne')`,
    ...
    await db.query(
      `INSERT INTO executions_consignation
       (point_id, numero_cadenas, mcc_ref, consigne_par, date_consigne)
       VALUES (?, ?, ?, ?, NOW())`,
      [point_id, numero_cadenas, mcc_ref, charge_id]
    );

    return success(res, { point_id, plan_id, numero_cadenas, mcc_ref, numero_ligne: nextLigne }, 'Cadenas enregistré');
  } catch (err) {
...
const validerConsignation = async (req, res) => {
  try {
    ...
    const [chargeInfo] = await db.query(
      'SELECT prenom, nom, matricule, badge_ocp_id FROM users WHERE id = ?',
      [charge_id]
    );
    ...
    await db.query(
      `UPDATE demandes_consignation
       SET statut = 'consigne', charge_id = ?, updated_at = NOW()
       WHERE id = ?`,
      [charge_id, id]
    );
    ...
    if (plan) {
      await db.query(
        `UPDATE plans_consignation SET statut = 'execute', updated_at = NOW() WHERE id = ?`,
        [plan.id]
      );
    }
    ...
    if (points.length > 0) {
      await db.query(
        `UPDATE points_consignation SET statut = 'verifie'
         WHERE plan_id = ? AND statut = 'consigne'`,
        [plan ? plan.id : 0]
      );
    }
    ...
    if (archiveExist.length > 0) {
      await db.query(
        'UPDATE dossiers_archives SET pdf_path = ?, cloture_par = ?, date_cloture = NOW() WHERE demande_id = ?',
        [pdfRelPath, charge_id, id]
      );
    } else {
      await db.query(
        `INSERT INTO dossiers_archives (demande_id, pdf_path, cloture_par, date_cloture, remarques)
         VALUES (?, ?, ?, NOW(), 'Consignation validée par le chargé')`,
...
// ─────────────────────────────────────────────
// GET /charge/demandes/:id/pdf  ✅ NOUVEAU
// Sert le fichier PDF — accessible par agent ET chargé
// ─────────────────────────────────────────────
const servirPDF = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      'SELECT pdf_path FROM dossiers_archives WHERE demande_id = ?',
      [id]
    );
    ...
    const pdfRelPath = rows[0].pdf_path;
...
const genererPDF = ({ demande, plan, points, charge, pdfPath, photoAbsPath }) => {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(7.5).font('Helvetica').fillColor('#000')
       .text(`Références des plans et schémas de l'installation ou équipement à consigner : ${plan?.schema_ref || demande.tag || ''}`,
             ML + 3, y + 3, { width: PW - 6 });
    y += 18;
    ...
    ORDERED.forEach((pt, i) => {
      ...
      if (pt) {
        cell(pt.numero_ligne,                 dx, C.num);    dx += C.num;
        cell(demande.tag || pt.repere_point,  dx, C.repere); dx += C.repere;
        cell(pt.mcc_ref || pt.localisation,   dx, C.local);  dx += C.local;
        cell('Cadenas',                       dx, C.disp);   dx += C.disp;
        cell('ouvert',                        dx, C.etat);   dx += C.etat;
        cell('électrique',                    dx, C.charge); dx += C.charge;
        cell(pt.numero_cadenas || '',         dx, C.cad);    dx += C.cad;
        cell(chargeNomComplet,                dx, C.cNom);   dx += C.cNom;
        cell(fmtDate(pt.date_consigne),       dx, C.cDate);  dx += C.cDate;
        cell(fmtHeureComplete(pt.date_consigne), dx, C.cHeure); dx += C.cHeure;
        cell(chargeNomComplet,                dx, C.vNom);   dx += C.vNom;
        cell(dateValidation,                  dx, C.vDate);  dx += C.vDate;
    ...
    doc.font('Helvetica').fontSize(7)
       .text(chargeNomComplet, ML + 4, y + 14);
    doc.font('Helvetica-Bold')
       .text('Date : ', ML + 4, y + 24, { continued: true })
       .font('Helvetica').text(fmtDate(now));
    doc.font('Helvetica-Bold')
    ...
    doc.font('Helvetica').fontSize(7)
       .text(chargeNomComplet, ML + basW + 4, y + 14);
    doc.font('Helvetica-Bold')
    ...
    if (photoAbsPath && fs.existsSync(photoAbsPath)) {
      ...
      doc.fontSize(7).font('Helvetica').fillColor('#555')
         .text(`Photo prise par ${chargeNomComplet} le ${fmtDate(now)} à ${fmtHeureComplete(now)}`,
