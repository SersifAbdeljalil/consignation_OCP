...
// toutes demandes confondues. Filtre par statut présence.
//
...
export default function MonEquipe({ navigation }) {
  ...
  const charger = async () => {
    try {
      ...
      const res = await getMesIntervenants();
  ...
  const renderCard = ({ item }) => {
    ...
    // Couleur statut présence
    let presenceColor = '#F59E0B'; // attente
      ...
      <View style={S.card}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={S.demandeRow}>
            ...
            <Text style={S.demandeTxt}>{item.numero_ordre}  ·  {item.tag}</Text>
          </View>
