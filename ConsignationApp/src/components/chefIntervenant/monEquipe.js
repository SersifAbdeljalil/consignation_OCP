...
export default function MonEquipe({ route, navigation }) {
  ...
  // demande passée en param OU sélectionnée depuis la liste
  ...
  // ── Charger liste des demandes consignées ──────────────────
  const chargerListe = useCallback(async () => {
    try {
      ...
      if (res?.success) {
        ...
        // Si une seule demande → aller directement au détail
        if (consignees.length === 1) {
          setDemandeSelectionnee(consignees[0]);
        }
      }
    } catch (e) {
          ...
          onPress: async () => {
            try {
              ...
              if (res.success) {
                ...
                Alert.alert('✅ Entrée enregistrée', `${enAttente.length} membre(s) sont maintenant sur site.`);
              } else {
  ...
  // ── Retour arrière selon le contexte ──────────────────────
  const handleBack = () => {
    if (params.demande) {
      // Venu d'un autre écran avec une demande précise → retour normal
      navigation.goBack();
    } else {
      // Venu du dashboard sans demande → retour à la liste
  ...
  // ══════════════════════════════════════════════════════════
  // RENDU MODE LISTE — aucune demande sélectionnée
  // ══════════════════════════════════════════════════════════
        ...
        {listeDemandes.length === 0 ? (
          <View style={[S.centered, { marginTop: 60 }]}>
            ...
            <Text style={S.emptySub}>
              Vos chantiers apparaîtront ici dès qu'une demande sera consignée.
            </Text>
          </View>
        ) : (
          <FlatList
  ...
  // ══════════════════════════════════════════════════════════
  // RENDU MODE DÉTAIL — demande sélectionnée
  // ══════════════════════════════════════════════════════════
  if (loading) {
    return (
      <View style={S.centered}>
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
      ...
      <View style={[S.header, { backgroundColor: CFG.couleur }]}>
        ...
        {equipeValidee ? (
          <TouchableOpacity
            style={S.backBtn}
            onPress={() => navigation.navigate('DeconsignationEquipe', { demande: demandeSelectionnee, userMetier })}
          >
            ...
            return (
              <View key={m.id} style={S.membreCard}>
                <View style={[S.avatar, { backgroundColor: sc.bg }]}>
                  <Text style={[S.avatarTxt, { color: sc.color }]}>
                    {(m.nom || '?')[0].toUpperCase()}
                  </Text>
                </View>
                <View style={{ flex: 1, marginLeft: 12 }}>
                  ...
                  <Text style={S.membreMeta}>
                    ...
                    {m.numero_cadenas ? `  ·  Cad: ${m.numero_cadenas}` : ''}
                  </Text>
                  {m.heure_entree && (
