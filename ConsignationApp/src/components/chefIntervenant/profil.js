// src/components/chefIntervenant/profil.js
...
const COLORS = {
  primary:     '#1565C0',   // bleu principal (méca)
  primaryDark: '#0D47A1',   // bleu foncé header
  primaryPale: '#E3F2FD',   // bleu très clair (fond badges)
  gray:        '#9E9E9E',
  grayDeep:    '#212121',
  grayDark:    '#424242',
  white:       '#FFFFFF',
...
export default function ProfilChef({ navigation }) {
  ...
  const chargerUser = async () => {
    ...
    } catch (e) { console.error('chargerUser error:', e); }
  };
  ...
  return (
    ...
    >
      ...
      {/* ══ HEADER ══ */}
      <View style={S.header}>
        {/* Déco circulaire (même style que dashboardChef) */}
        <View style={{
        ...
        <View style={S.roleRow}>
          <Ionicons
            name={ROLE_ICON[user?.role] || 'people-outline'}
            size={13}
      ...
      >
        ...
        <View style={S.card}>
          ...
          <InfoItem iconName="card-outline"      label="Matricule" value={user?.matricule || '—'} />
          <InfoItem iconName="business-outline"  label="Entité"    value={user?.entite || '—'} />
          <InfoItem iconName="briefcase-outline" label="Rôle"      value={ROLE_LABEL[user?.role] || user?.role || '—'} />
          {user?.zone && (
            <InfoItem iconName="map-outline" label="Zone" value={user.zone} />
          )}
        </View>
        ...
        <View style={S.card}>
          ...
          {telErreur ? (
            ...
            <View style={S.infoSmsRow}>
              ...
              <Text style={S.infoSmsText}>Un SMS de vérification sera envoyé après enregistrement</Text>
            </View>
          )}
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[S.paysOptionLabel, paysIndex === index && { color: COLORS.primary }]}>
                      {item.label}
                    </Text>
...
const S = StyleSheet.create({
  ...
  headerRole:    { fontSize: 12, color: 'rgba(255,255,255,0.75)', letterSpacing: 0.5 },
  matriculeRow:  { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 2 },
  matriculeText: { fontSize: 11, color: 'rgba(255,255,255,0.6)' },
  ...
  cardIconWrap: {
    ...
    backgroundColor: COLORS.primaryPale,   // 🔵 bleu pâle
    alignItems: 'center', justifyContent: 'center',
  },
  ...
  telActuelText: { fontSize: 12, color: COLORS.primary, fontWeight: '600' },  // 🔵
  ...
  verifTitle:  { fontSize: 14, fontWeight: '700', color: COLORS.primary },  // 🔵
  verifSub:    { fontSize: 12, color: '#1565C0', marginBottom: 12, lineHeight: 17 },
  codeInput: {
    backgroundColor: '#fff',
    ...
    letterSpacing: 12, marginBottom: 12, textAlign: 'center',
  },
  btnVerif: {
  ...
  paysOptionSel:   { backgroundColor: COLORS.primaryPale },   // 🔵
