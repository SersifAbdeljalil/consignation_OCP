...
try {
  ...
  if (TcpSocket?.default) TcpSocket = TcpSocket.default;
} catch (e) {
...
export default function PdfViewer({ navigation, route }) {
  ...
  const [serverActif, setServerActif] = useState(false);

  useEffect(() => {
    chargerPDF();
    return () => { arreterServeur(); };
  }, []);

  useEffect(() => {
    if (!qrVisible) arreterServeur();
  }, [qrVisible]);
  ...
  const demarrerServeur = async () => {
    ...
    if (!TcpSocket) {
      Alert.alert(
        ...
        'Le partage QR nécessite un development build.\nUtilisez le bouton ⬇️ télécharger à la place.'
      );
      return;
    }
      ...
      <head>
        ...
        <style>
          * { margin:0; padding:0; box-sizing:border-box; }
          html, body { width:100%; height:100%; background:#525659; overflow-x:hidden; overflow-y:auto; }
          #pages { display:flex; flex-direction:column; align-items:center; padding:12px 0; gap:10px; }
          canvas { display:block; width:100% !important; box-shadow:0 2px 12px rgba(0,0,0,0.6); background:white; }
          #msg { color:#fff; font-family:sans-serif; font-size:14px; text-align:center; padding:60px 20px; }
          #progress-wrap {
            position:fixed; bottom:0; left:0; right:0;
            background:rgba(0,0,0,0.5); padding:8px 16px;
            display:flex; align-items:center; gap:10px;
          }
          #progress-bar-bg { flex:1; height:6px; background:rgba(255,255,255,0.2); border-radius:3px; overflow:hidden; }
          #progress-bar { height:100%; background:${theme.couleur}; border-radius:3px; transition:width 0.2s; width:0%; }
          #progress-txt { color:#fff; font-family:sans-serif; font-size:11px; min-width:60px; text-align:right; }
        </style>
      </head>
      <body>
            ...
            const renderPage = (num) => {
              pdf.getPage(num).then(page => {
                ...
                page.render({ canvasContext: ctx, viewport: vp }).promise.then(() => {
                  ...
                  if (rendered === total) setTimeout(() => { progressWrap.style.display = 'none'; }, 800);
      ...
      <View style={[S.header, { backgroundColor: theme.couleur }]}>
        ...
        {etat === 'affichage' && (
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity style={S.headerBtn} onPress={ouvrirQr}>
