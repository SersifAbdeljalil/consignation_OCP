...
// ✅ Nouveau composant — liste des demandes actives du chargé
// ✅ Lecture de filtreInitial depuis route.params (venant du dashboard)
// ✅ Navigation vers DetailConsignation au clic
// ✅ Filtres : Toutes / En attente / En cours / Att.Process / Att.Chargé
...
const FILTRES = [
  ...
  { key: 'consigne_charge', label: 'Att. Process', icon: 'hourglass-outline'    },
  { key: 'consigne_process',label: 'Att. Chargé',  icon: 'hourglass-outline'    },
];
...
export default function MesDemandesCharge({ navigation, route }) {
  ...
  // ✅ Si on revient avec un nouveau filtreInitial
  useEffect(() => {
  ...
  const charger = useCallback(async () => {
    try {
      const res = await getDemandesAConsigner();
  ...
  const renderCard = ({ item }) => {
    ...
    return (
      ...
      >
        ...
        {/* Indicateur de progression pour consigne_charge / consigne_process */}
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
      ...
      <View style={[S.statsBar, { backgroundColor: CFG.couleur }]}>
        {[
          ...
          { lbl: 'Att.Proc.',  val: stats.consigne_charge,    color: '#BFDBFE' },
        ].map((s, i) => (
