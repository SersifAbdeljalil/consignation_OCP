...
// ✅ Profil conservé dans les actions rapides
// ✅ Tuile "Déconsigner" → MesDemandesProcess avec filtre 'deconsignation'
// ✅ Bannière urgente si déconsignations en attente
...
import { getDemandesProcess, getDemandesADeconsignerProcess } from '../../api/process.api';
...
export default function DashboardProcess({ navigation }) {
  ...
  const [aDeconsigner, setADeconsigner] = useState([]);
  ...
  const charger = useCallback(async () => {
    try {
      ...
      if (deconRes?.success)    setADeconsigner(deconRes.data || []);
  ...
  useEffect(() => { isMountedRef.current = true; charger(); return () => { isMountedRef.current = false; }; }, [charger]);
  useEffect(() => { intervalRef.current = setInterval(chargerSilencieux, REFRESH_INTERVAL_MS); return () => clearInterval(intervalRef.current); }, [chargerSilencieux]);
  useEffect(() => { const unsub = navigation.addListener('focus', charger); return unsub; }, [navigation, charger]);
  ...
  const stats = {
    ...
    aDeconsigner: aDeconsigner.length,
  };
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
      ...
      >
        ...
        <View style={[S.header, { backgroundColor: CFG.couleur }]}>
          ...
          <TouchableOpacity style={S.notifBtn} onPress={() => navigation.navigate('NotificationsProcess')}>
            <Ionicons name="notifications-outline" size={22} color="#fff" />
            {notifCount > 0 && (
              <View style={S.notifBadge}><Text style={S.notifBadgeTxt}>{notifCount > 9 ? '9+' : notifCount}</Text></View>
            )}
          </TouchableOpacity>
        </View>
        ...
        <View style={S.statsRow}>
          {[
            { label: 'En attente',  value: stats.en_attente,   color: '#F59E0B', icon: 'time-outline',        filtre: 'en_attente'     },
            { label: 'En cours',    value: stats.en_cours,     color: CFG.couleur, icon: 'sync-outline',      filtre: 'en_cours'       },
            { label: 'Déconsigner', value: stats.aDeconsigner, color: '#7C3AED', icon: 'cog-outline',         filtre: 'deconsignation' },
            { label: 'Total',       value: stats.total,        color: '#6B7280', icon: 'list-circle-outline', filtre: null             },
          ].map((s, i) => (
            <TouchableOpacity
              key={i}
              style={S.statCard}
              onPress={() => navigation.navigate('MesDemandesProcess', { filtreInitial: s.filtre })}
              activeOpacity={0.75}
            >
              ...
              {s.label === 'Déconsigner' && stats.aDeconsigner > 0 && (
                <View style={[S.urgentDot, { backgroundColor: '#7C3AED' }]} />
              )}
        ...
        {stats.aDeconsigner > 0 && (
          <TouchableOpacity
            style={S.deconBanner}
            onPress={() => navigation.navigate('MesDemandesProcess', { filtreInitial: 'deconsignation' })}
            activeOpacity={0.85}
          >
            <View style={[S.deconBannerIcon, { backgroundColor: '#EDE9FE' }]}>
              <Ionicons name="cog-outline" size={20} color="#7C3AED" />
            </View>
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={S.deconBannerTitle}>
                {stats.aDeconsigner} déconsignation{stats.aDeconsigner > 1 ? 's' : ''} process en attente
              </Text>
              <Text style={S.deconBannerSub}>Vannes à déconsigner — action requise</Text>
            </View>
            <Ionicons name="chevron-forward" size={18} color="#7C3AED" />
          </TouchableOpacity>
        )}

        {/* Actions rapides — 4 boutons AVEC PROFIL */}
        <Text style={S.sectionTitle}>Actions rapides</Text>
        <View style={S.actionsGrid}>
          {[
            { icon: 'list-outline',          label: 'Consignations',   sub: `${demandes.length} à traiter`,    screen: 'MesDemandesProcess',  params: {}                               },
            { icon: 'cog-outline',           label: 'Déconsignations', sub: `${stats.aDeconsigner} en attente`, screen: 'MesDemandesProcess',  params: { filtreInitial: 'deconsignation' }, urgent: stats.aDeconsigner > 0 },
            ...
            <TouchableOpacity
              key={i}
              style={[S.actionCard, a.urgent && S.actionCardUrgent]}
              onPress={() => navigation.navigate(a.screen, a.params)}
              activeOpacity={0.8}
            >
              <View style={[S.actionIcon, { backgroundColor: a.urgent ? '#EDE9FE' : CFG.bgPale }]}>
                <Ionicons name={a.icon} size={22} color={a.urgent ? '#7C3AED' : CFG.couleur} />
              </View>
              <Text style={S.actionLabel}>{a.label}</Text>
              <Text style={[S.actionSub, a.urgent && { color: '#7C3AED', fontWeight: '700' }]}>{a.sub}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={S.sectionRow}>
          <Text style={[S.sectionTitle, { marginHorizontal: 0, marginBottom: 0 }]}>Dernières demandes</Text>
              ...
              >
                ...
                <View style={{ flex: 1, marginLeft: 12 }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2 }}>
                    ...
                    <Text style={S.demandeTag}> {d.tag || ''}{d.equipement_nom ? ` — ${d.equipement_nom}` : ''}</Text>
                  </View>
...
const S = StyleSheet.create({
  header: { paddingTop: 50, paddingBottom: 30, paddingHorizontal: 16, borderBottomLeftRadius: 30, borderBottomRightRadius: 30, overflow: 'hidden' },
  headerDecoCircle: { position: 'absolute', bottom: -30, right: -30, width: 120, height: 120, borderRadius: 60, backgroundColor: 'rgba(255,255,255,0.15)' },
  ...
  notifBtn:       { position: 'absolute', top: 52, right: 16, width: 40, height: 40, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  ...
  statsRow:       { flexDirection: 'row', gap: 8, marginHorizontal: 16, marginTop: -20, marginBottom: 12 },
  statCard:       { flex: 1, backgroundColor: '#fff', borderRadius: 14, padding: 10, alignItems: 'center', elevation: 5, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 } },
  statVal:        { fontSize: 20, fontWeight: '900' },
  statLbl:        { fontSize: 8, color: '#9E9E9E', marginTop: 2, textAlign: 'center' },
  urgentDot:      { position: 'absolute', top: 6, right: 6, width: 8, height: 8, borderRadius: 4 },
  statArrow:      { position: 'absolute', bottom: 6, right: 6, width: 14, height: 14, borderRadius: 7, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  deconBanner:     { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 12, backgroundColor: '#F5F3FF', borderRadius: 14, padding: 12, borderWidth: 1.5, borderColor: '#C4B5FD' },
  deconBannerIcon: { width: 40, height: 40, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  deconBannerTitle:{ fontSize: 13, fontWeight: '800', color: '#5B21B6' },
  deconBannerSub:  { fontSize: 11, color: '#7C3AED', marginTop: 2 },
  sectionRow:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: 16, marginTop: 4, marginBottom: 10 },
  sectionTitle:    { fontSize: 14, fontWeight: '700', color: '#424242', marginHorizontal: 16, marginBottom: 10, marginTop: 4 },
  sectionCount:    { fontSize: 11, color: '#9E9E9E' },
  actionsGrid:     { flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: 16, gap: 10, marginBottom: 16 },
  actionCard:      { backgroundColor: '#fff', borderRadius: 14, padding: 14, width: '47%', alignItems: 'center', elevation: 3, shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } },
  actionCardUrgent:{ borderWidth: 1.5, borderColor: '#C4B5FD' },
  ...
  demandeCard:     { backgroundColor: '#fff', borderRadius: 14, padding: 14, marginHorizontal: 16, marginBottom: 10, flexDirection: 'row', alignItems: 'center', elevation: 3, shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } },
  ...
  demandeTag:      { fontSize: 11, color: '#b45309', fontWeight: '600' },
  ...
  voirToutBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 16, marginTop: 4, marginBottom: 8, backgroundColor: '#fde68a', borderRadius: 14, paddingVertical: 12, borderWidth: 1, borderColor: '#fcd34d' },
