...
import { validerConsignationProcess } from '../../api/process.api';
...
export default function ValiderProcess({ navigation, route }) {
  ...
  const tousProcessOk = pointsProcess.length === 0 || pointsProcess.every(p =>
    p.numero_cadenas || p.saved
  );
  const peutValider = tousProcessOk && badgeValide;

  const handleValider = () => {
    Alert.alert(
      'Confirmer la validation process',
      `Voulez-vous valider la consignation process de ${demande.tag} ?\n\nLes notifications seront envoyees a l'agent et aux chefs intervenants.`,
      [
      ...
      <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 30 }}>
          ...
          <View style={{ width: '100%', gap: 10, marginTop: 10 }}>
            {[
              ...
              { icon: 'cog-outline',             txt: `${pointsProcess.length} cadenas process poses`   },
        ...
        <View style={S.bottomBar}>
          ...
          >
            ...
            <Text style={S.btnTxt}>VOIR LE DÉTAIL & PDF</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[S.btn, { backgroundColor: '#6B7280' }]}
            onPress={() => navigation.navigate('DashboardProcess')}
            activeOpacity={0.85}
          >
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#000' }}>
        ...
        <View style={S.stepperCam}>
          {['Cadenas', 'Valider'].map((s, i) => (
            <View key={i} style={S.stepItem}>
              <View style={[
                S.stepCircle,
                i < 1 && { backgroundColor: '#10B981' },
                i === 1 && { backgroundColor: CFG.couleur },
              ]}>
                {i < 1
              ...
              <Text style={[S.stepLbl, i === 1 && { color: '#fff', fontWeight: '700' }]}>{s}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={S.camInstructions}>
          ...
          <View style={[S.instructCard, { backgroundColor: 'rgba(180,83,9,0.92)' }]}>
            <Ionicons name="shield-checkmark-outline" size={24} color="#fff" />
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={S.instrTitle}>Confirmez votre identite</Text>
              <Text style={S.instrSub}>
                Placez votre badge OCP dans le cadre pour signer la consignation process
              </Text>
            </View>
          </View>
        </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
      ...
      <View style={[S.header, { backgroundColor: CFG.couleur }]}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={S.hTitle}>Etape 2 / 2 — Validation</Text>
          <Text style={S.hSub}>Verification finale process</Text>
        </View>
        <View style={{ width: 36 }} />
      </View>
      ...
      <View style={S.stepper}>
        {['Cadenas', 'Valider'].map((s, i) => (
          <View key={i} style={S.stepItem}>
            ...
            <Text style={[S.stepLbl, i === 1 && { color: CFG.couleur, fontWeight: '700' }]}>{s}</Text>
          </View>
        ))}
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 130 }}>
        ...
        <View style={S.card}>
          ...
          {[
            { icon: 'cog-outline',  lbl: `${pointsProcess.length} cadenas process scannes`, ok: tousProcessOk },
            { icon: 'card-outline', lbl: badgeValide ? `Badge confirme : ${badgeId}` : 'Badge non encore scanne', ok: badgeValide },
          ].map((c, i) => (
            <View key={i} style={[S.checkRow, i < 1 && { borderBottomWidth: 1, borderBottomColor: '#F5F5F5' }]}>
          ...
          {badgeValide ? (
            ...
            <View>
              <Text style={S.badgeInfo}>
                Scannez votre badge OCP pour signer officiellement la consignation process avant de valider.
              </Text>
              <TouchableOpacity
            ...
            <View style={{ marginTop: 10, flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
              {demande.types_intervenants.map((t, i) => (
                <View key={i} style={[S.typeChip, {
                  backgroundColor: t === 'process' ? CFG.bgPale : '#F5F5F5',
                  borderColor: t === 'process' ? CFG.couleur : '#E0E0E0',
                }]}>
                  <Text style={[S.typeChipTxt, { color: t === 'process' ? CFG.couleur : '#9E9E9E' }]}>
                    {TYPE_LABEL[t] || t}
                  </Text>
                </View>
              ))}
        ...
        {/* Cadenas process scannés */}
        {pointsProcess.length > 0 && (
          <View style={[S.card, { marginTop: 14 }]}>
            <View style={S.cardTitleRow}>
              <Ionicons name="cog-outline" size={16} color={CFG.couleur} />
              <Text style={S.cardTitle}>Cadenas process ({pointsProcess.length})</Text>
            </View>
            {pointsProcess.map((pt, i) => (
              <View key={i} style={[S.pointRow, i < pointsProcess.length - 1 && { borderBottomWidth: 1, borderBottomColor: '#F5F5F5' }]}>
                <Ionicons name="lock-closed" size={14} color={CFG.couleur} />
                <View style={{ flex: 1, marginLeft: 8 }}>
                  <Text style={S.pointRepere}>
                    {pt.repere || pt.repere_point} — {pt.dispositif || pt.dispositif_condamnation}
                  </Text>
        ...
        {/* Points electrique — info seulement */}
        {pointsElec.length > 0 && (
          <View style={[S.card, { marginTop: 14, opacity: 0.7 }]}>
            <View style={S.cardTitleRow}>
              <Ionicons name="flash-outline" size={16} color="#9E9E9E" />
              <Text style={[S.cardTitle, { color: '#9E9E9E' }]}>Points Electrique ({pointsElec.length})</Text>
            </View>
            <View style={[S.processInfo, { backgroundColor: '#F5F5F5' }]}>
              <Ionicons name="information-circle-outline" size={14} color="#9E9E9E" />
              <Text style={[S.processInfoTxt, { color: '#9E9E9E' }]}>Ces points sont geres par le Charge de consignation</Text>
            </View>
          </View>
        )}
        ...
        <View style={[S.pdfInfo, { backgroundColor: CFG.bgPale, borderColor: CFG.couleur, marginTop: 14 }]}>
          <Ionicons name="notifications-outline" size={20} color={CFG.couleur} />
          <Text style={[S.pdfInfoTxt, { color: CFG.couleurDark }]}>
            En cliquant VALIDER, les notifications seront envoyees a l'agent demandeur
            et aux chefs intervenants concernes. Aucun PDF n'est genere par cette etape.
          </Text>
        </View>
          ...
          {loading
            ...
            : (
              <>
                ...
                <Text style={S.btnTxt}>
                  {peutValider ? 'VALIDER LA CONSIGNATION PROCESS' : 'SCANNEZ VOTRE BADGE D\'ABORD'}
                </Text>
              </>
            )
          }
...
const S = StyleSheet.create({
  ...
  stepper:    { flexDirection: 'row', justifyContent: 'center', paddingVertical: 14, backgroundColor: '#fff', gap: 40, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  ...
  processInfo:    { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 8, padding: 8 },
  processInfoTxt: { flex: 1, fontSize: 11 },
  ...
  corner:    { position: 'absolute', width: 26, height: 26, borderColor: '#b45309', borderWidth: 3 },
